#Exercise 2
#building a cifar classifier using pretrained resnet18 but finetuning the model, where the last layer should be FC(linear layer)
...
print(model) #we can visualize the layers inside the model
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = model.to(device)

for param in model.parameters():
    param.requires_grad=True #because we are training the whole network
